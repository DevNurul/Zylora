...
export default function PaymentCallback() {
    ...
    ;(async () => {
      try {
        ...
        if (data.paymentStatus === 'PAYMENT_SUCCESS') {
          dispatch(setOrder({
            orderId: data.orderId, fullName: data.customerName, email: data.email, total: data.total, estimatedDelivery: data.estimatedDelivery,
            items: (data.items || []).map((item) => ({ id: item.productId, name: item.name, price: item.price, qty: item.qty, size: item.size, color: item.color, image: item.image })),
          }))
          dispatch(clearCart())
        ...
        else { setStatus(STATUS.FAILED); setErrorMsg('Your payment could not be completed.') }
      } catch { setStatus(STATUS.FAILED); setErrorMsg('Unable to verify payment. Please contact support.') }
    })()
  ...
  if (!transactionId) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-5 px-4">
        ...
        <button onClick={() => navigate('/')} className="border border-[#B8976A] text-[#B8976A] px-8 py-3 text-sm hover:bg-[#B8976A] hover:text-white transition-colors rounded-xl">
          Go Home
        </button>
      </div>
    )
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-5 px-4 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-2">
          <button onClick={() => navigate('/track-order')} className="border border-[#B8976A] text-[#B8976A] px-8 py-3 text-sm hover:bg-[#B8976A] hover:text-white transition-colors rounded-xl">
            Track Order
          </button>
  ...
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center gap-5 px-4 text-center">
      ...
      <p className="text-[#9A9A9A] text-sm max-w-xs">{errorMsg}</p>
      <div className="flex flex-col sm:flex-row gap-3 mt-2">
        <button onClick={() => navigate('/checkout')} className="border border-[#B8976A] text-[#B8976A] px-8 py-3 text-sm hover:bg-[#B8976A] hover:text-white transition-colors rounded-xl">
          Try Again
        </button>
        <button onClick={() => navigate('/')} className="bg-gradient-to-r from-[#E8A0B0] to-[#D48A9A] text-white px-8 py-3 text-sm hover:shadow-[0_8px_30px_rgba(238,107,131,0.3)] transition-all rounded-xl">
          Go Home
